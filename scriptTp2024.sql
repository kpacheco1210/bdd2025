...
BEGIN
    ...
    -- TABLAS SEGÚN DIAGRAMA: Pedido y Detalle_pedido
    CREATE TABLE [GeDeDe].[Pedido] (
    ...
    CREATE TABLE [GeDeDe].[Detalle_pedido] (
        detP_codigo INT PRIMARY KEY,
        detP_cantidad BIGINT,
        detP_precio DECIMAL(18,2),
        detP_pedido INT,
        detP_subtotal DECIMAL(18,2),
        FOREIGN KEY (detP_pedido) REFERENCES [GeDeDe].[Pedido](ped_codigo)
    );
END
GO
...
BEGIN
    ...
    print 'Cargando tabla Pedido'
    BEGIN
        INSERT INTO [GD1C2024].[GeDeDe].[Pedido] (ped_codigo, ped_sucursal, ped_cliente, ped_fecha, ped_total, ped_estado, ped_cancelacion, ped_numero)
        SELECT DISTINCT 
            [PEDIDO_CODIGO],
            [SUCURSAL_CODIGO],
            [CLIENTE_CODIGO],
            ...
            [PEDIDO_NUMERO]
        FROM [GD1C2024].[gd_esquema].[Maestra]
        WHERE [PEDIDO_CODIGO] IS NOT NULL

    END;

    print 'Cargando tabla Detalle_pedido'
    BEGIN
        -- Ejemplo de carga, debes ajustar los campos y origen según tu fuente real
        INSERT INTO [GD1C2024].[GeDeDe].[Detalle_pedido] (detP_codigo, detP_cantidad, detP_precio, detP_pedido, detP_subtotal)
        SELECT DISTINCT 
            [DETALLE_CODIGO],
            [DETALLE_CANTIDAD],
            [DETALLE_PRECIO],
            ...
            [DETALLE_SUBTOTAL]
        FROM [GD1C2024].[gd_esquema].[Maestra]
        WHERE [DETALLE_CODIGO] IS NOT NULL
